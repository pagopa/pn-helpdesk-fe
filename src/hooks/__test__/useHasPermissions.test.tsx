  ...
  useEffect(() => {
    if (props.doLogUser) {
      setCurrentUser({
        ...
        permissions: [Permission.API_KEY_READ, Permission.API_KEY_WRITE, Permission.LOG_EXTRACT_READ],
      });
    }
...
describe("useHasPermission hook", () => {
  ...
  it('ask a permission owned by the logged user', async () => {
    await act(async () => void render(<FakeIndex requiredPermissions={[Permission.API_KEY_WRITE]} doLogUser={true} />));
    const allowedComponent = screen.queryByTestId('allowed');
    const blockedComponent = screen.queryByTestId('blocked');
    expect(allowedComponent).toBeInTheDocument();
    expect(blockedComponent).not.toBeInTheDocument();
  ...
  it('ask a permission not owned by the logged user', async () => {
    await act(async () => void render(<FakeIndex requiredPermissions={[Permission.LOG_DOWNTIME_READ]} doLogUser={true} />));
    const allowedComponent = screen.queryByTestId('allowed');
    const blockedComponent = screen.queryByTestId('blocked');
    expect(allowedComponent).not.toBeInTheDocument();
    expect(blockedComponent).toBeInTheDocument();
  ...
  it('ask two permissions owned by the logged user', async () => {
    await act(async () => void render(<FakeIndex requiredPermissions={[Permission.API_KEY_WRITE, Permission.API_KEY_READ]} doLogUser={true} />));
    const allowedComponent = screen.queryByTestId('allowed');
    const blockedComponent = screen.queryByTestId('blocked');
    expect(allowedComponent).toBeInTheDocument();
    expect(blockedComponent).not.toBeInTheDocument();
  ...
  it('ask two permissions, one is owned by the logged user, the other one is not', async () => {
    await act(async () => void render(<FakeIndex requiredPermissions={[Permission.TENDER_READ, Permission.API_KEY_READ]} doLogUser={true} />));
    const allowedComponent = screen.queryByTestId('allowed');
    const blockedComponent = screen.queryByTestId('blocked');
    expect(allowedComponent).not.toBeInTheDocument();
    expect(blockedComponent).toBeInTheDocument();
