...
const MenuItems: { [key: string]: Array<string> } = {
  ...
  'Ottieni log di sessione': ['ticketNumber', 'jti', 'Time interval', 'deanonimization'],
};
 ...
 */
const FieldsProperties: { [key: string]: FieldsProps } = {
  'Tipo Estrazione': {
  ...
  Month: {
    ...
    rules: {
      ...
      validate: {
        required: (value: string) => value !== 'Invalid date' || errorMessages.REQUIRED,
      },
    },
    view: ['month', 'year'],
    type: 'month',
  ...
  'Month Interval': {
    ...
    maxDate: format(
      new Date(
        new Date(
          new Date(new Date().setUTCMonth(new Date().getMonth() + 1)).setHours(0, 0, 0, 0)
        ).setUTCDate(0)
      ),
      ...
      validate: {
        validateInterval: (dates: Array<any>) => {
          ...
          const maxDate = new Date(
            new Date(new Date(startDate).setUTCDate(2)).setUTCMonth(startDate.getUTCMonth() + 1)
          );
    ...
    rules: {
      ...
      validate: {
        validateInterval: (dates: Array<any>) => {
          const startDate = new Date(dates[0]);
          const endDate = new Date(dates[1]);
          const minStartDate = subMonths(endDate, 3);
          return (
