...
describe("PaAssociation tests", () => {
    ...
    beforeEach(async () => {
        ...
        const items = pa_list_associated.items.map(
    ...
    it("check pa and save", async () => {
        ...
        await waitFor(() => expect(screen.getByRole("button", {name : "Conferma"})).toBeInTheDocument());

        //Click confirm in dialog
        fireEvent.click(screen.getByRole("button", {name : "Conferma"}));
        await waitFor(() => expect(navigate).toHaveBeenCalledWith(routes.GET_UPDATE_AGGREGATE_PATH(idAggregate)));
