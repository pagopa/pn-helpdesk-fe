...
describe("PaTransferListPage test", () => {
    ...
    it("renders", async () => {
        renderWithProviders(<ConfirmationProvider><PaTransferListPage email="test" /></ConfirmationProvider>)
    ...
    it("select aggregates", async () => {
        ...
        const paListContainer = screen.getAllByRole("list");
        let firstAggregateId = aggregates_list.items[0].id;
        let firstAggregateName = aggregates_list.items[0].name;
        let secondAggregateName = aggregates_list.items[1].name;
        let secondAggregateId = aggregates_list.items[1].id;
        ...
        await waitFor(() => expect(apiSpyGetAssociatedPaList).toBeCalled());
        const paListSecondAggregate = await waitFor(() => within(paListContainer[1]).getAllByRole("listitem"));
