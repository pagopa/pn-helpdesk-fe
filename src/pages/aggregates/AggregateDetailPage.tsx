...
const AggregateDetailPage = ({ email }: any) => {
  ...
  useEffect(() => {
    ...
    const requestList = [apiRequests.getUsagePlans()];
  ...
  const handleClickAdd = () => {
    navigate(routes.ADD_PA, {
      state: { aggregate: { ...aggregate, associatedPa: pas } as getAggregateResponse },
    });
  };

  const handleClickTransfer = () => {
    navigate(routes.TRANSFER_PA, {
      state: { aggregate: { id: idAggregate, name: aggregate?.name } },
    });
  };
